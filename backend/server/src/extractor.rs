...
use tracing::info;
use socialverse_domain::ids::{SessionId, UserId};
use socialverse_endpoint::RequestFailed;
use socialverse_query::OwnedAsyncConnection;
    ...
    S: Send + Sync,
{
    ...
    async fn from_request_parts(parts: &mut Parts, _: &S) -> Result<Self, Self::Rejection> {
        ...
        let session_id = socialverse_cookie::get_from_str(cookies, socialverse_cookie::SESSION_ID)
            .and_then(|id| SessionId::from_str(id).ok())
            .ok_or_else(unauthorized)?;

        let session_signature =
            socialverse_cookie::get_from_str(cookies, socialverse_cookie::SESSION_SIGNATURE)
                .and_then(|sig| socialverse_crypto::decode_base64(sig).ok())
                .and_then(|sig| socialverse_crypto::sign::signature_from_bytes(sig).ok())
                .ok_or_else(unauthorized)?;
        ...
        let session = socialverse_query::session::get(&mut conn, session_id)
            .ok()
            .flatten()
            .ok_or_else(unauthorized)?;
