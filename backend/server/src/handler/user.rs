...
use tracing::info;
use socialverse_domain::{ids::*, user::DisplayName};
...
fn profile_id_to_url(id: &str) -> Url {
    use socialverse_endpoint::app_url::{self, user_content};
...
pub fn to_public(
    conn: &mut socialverse_query::AsyncConnection,
    ...
    Ok(PublicUserProfile {
        ...
        am_following: {
            match session {
                Some(session) => socialverse_query::user::is_following(conn, session.user_id, user.id)?,
                None => false,
            }
        },
    })
...
fn new_session(
    ...
    let session = socialverse_query::session::new(conn, user_id, session_duration, fingerprint.into())?;
    ...
    let signature = socialverse_crypto::encode_base64(signature);
...
impl PublicApiRequest for CreateUser {
    ...
    async fn process_request(
        ...
        let password_hash = socialverse_crypto::hash_password(&self.password)?;
        let user_id = socialverse_query::user::new(&mut conn, password_hash, &self.username)
...
impl PublicApiRequest for Login {
    ...
    async fn process_request(
        ...
        let hash = socialverse_query::user::get_password_hash(&mut conn, &self.username)
            .map_err(|_| ServerError::wrong_password())?;

        let hash = socialverse_crypto::password::deserialize_hash(&hash)
            .map_err(|_| ServerError::wrong_password())?;

        socialverse_crypto::verify_password(self.password, &hash)
            .map_err(|_| ServerError::wrong_password())?;

        let user = socialverse_query::user::find(&mut conn, &self.username)
...
impl AuthorizedApiRequest for GetMyProfile {
    ...
    async fn process_request(
        ...
        let user = socialverse_query::user::get(&mut conn, session.user_id)?;
    ...
    async fn process_request(
        ...
        let password = {
            if let Update::Change(ref password) = payload.password {
                Update::Change(socialverse_crypto::hash_password(password)?)
            } else {
                Update::NoChange
            }
        };
        ...
        socialverse_query::user::update_profile(&mut conn, query_params)?;
...
impl AuthorizedApiRequest for ViewProfile {
    ...
    async fn process_request(
        ...
        let profile = socialverse_query::user::get(&mut conn, self.for_user)?;
        let profile = to_public(&mut conn, Some(&session), profile)?;

        let mut posts = vec![];

        for post in socialverse_query::post::get_public_posts(&mut conn, self.for_user)? {
            let post_id = post.id;
...
impl AuthorizedApiRequest for FollowUser {
    ...
    async fn process_request(
        ...
        match self.action {
            FollowAction::Follow => {
                socialverse_query::user::follow(&mut conn, session.user_id, self.user_id)?;
            }
            FollowAction::Unfollow => {
                socialverse_query::user::unfollow(&mut conn, session.user_id, self.user_id)?;
            }
        }
